...
TABLES_COUNT=$($MYSQL -u $LOCAL_DB_USER $MDB -e "SELECT count(*) from information_schema.tables WHERE table_schema = 'stories';" | $AWK '{print $1}' | $GREP -v '^count')
$ECHO $TABLES_COUNT 'tables detected into stories database'

#executes the script
$MYSQL -u $LOCAL_DB_USER -D $MDB < $MAIN_PATH/tool/$TEST_DUMP_DATABASE_TABLES 

#check if database is empty
TABLES_COUNT_AFTER_DELETE=$($MYSQL -u $LOCAL_DB_USER $MDB -e "SELECT count(*) from information_schema.tables WHERE table_schema = 'stories';" | $AWK '{print $1}' | $GREP -v '^count')
