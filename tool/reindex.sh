...
if [[ $TARGET == 'localdev' ]] || [[ $TARGET == 'verify' ]]; then
	...
	DB_STRING="jdbc:mysql://${MYSQL_PRODUCTION}/$PROD_DB?user=$DB_USER"
    elif [[ $TARGET == 'testing' ]]; then
	DB_STRING="jdbc:mysql://${MYSQL_STAGING}/$TEST_DB?user=$DB_USER"
    elif [[ $TARGET == 'integration' ]]; then
	DB_STRING="jdbc:mysql://${MYSQL_STAGING}/$INT_DB?user=$DB_USER"
    else # Default to the 'verify' DB as the most volatile / least important.
	DB_STRING="jdbc:mysql://${MYSQL_STAGING}/$VERIFY_DB?user=$DB_USER"
    fi
