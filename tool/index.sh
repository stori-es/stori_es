...
if [[ $TARGET == 'localdev' ]] || [[ $TARGET == 'verify' ]]; then
    ...
    if [[ $TARGET == 'localdev' ]]; then
        ...
        DB_STRING="jdbc:mysql://${MYSQL_STAGING}/$VERIFY_DB?user=$TEST_DB_USER&password=`ensure-db-password`"
	...
	DB_STRING='jdbc:mysql://${MYSQL_STAGING}/$VERIFY_DB?user=$TEST_DB_USER'
    fi
