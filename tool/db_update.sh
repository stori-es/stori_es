...
do
  ...
  FILENAME=${f##*/}
  IS_EXECUTED=$($MYSQL -u $LOCAL_DB_USER -e "SELECT count(*) FROM dbUpdate db WHERE LOWER(db.scriptName) = LOWER('"$FILENAME"')" $MDB | $AWK '{print $1}' | $GREP -v '^count')
  if [ $IS_EXECUTED = 0 ]; then 
  	$MYSQL -u $LOCAL_DB_USER $MDB < $f
  	if [ $? = 0 ] 
  	then
   		echo "Script Executed Successful " $f
   		$MYSQL -u $LOCAL_DB_USER -e "insert into dbUpdate (scriptName) values ('"$FILENAME"')" $MDB
   	else
