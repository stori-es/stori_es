...
TABLES_COUNT=$($MYSQL -u $LOCAL_DB_USER $MDB -e "SELECT count(*) from information_schema.tables WHERE table_schema = '$MDB';" | $AWK '{print $1}' | $GREP -v '^count')
...
$MYSQLDUMP -u $LOCAL_DB_USER --add-drop-table --no-data $MDB | $GREP ^DROP >> $TEST_DUMP_DATABASE_TABLES
$ECHO 'SET FOREIGN_KEY_CHECKS = 1;' >> $TEST_DUMP_DATABASE_TABLES

#executes the script
$MYSQL -u $LOCAL_DB_USER $MBD < $TEST_DUMP_DATABASE_TABLES

#delete the tmp file that stores the script
$RM $TEST_DUMP_DATABASE_TABLES

#check if database is empty
TABLES_COUNT_AFTER_DELETE=$($MYSQL -u $LOCAL_DB_USER $MDB -e "SELECT count(*) from information_schema.tables WHERE table_schema = '$MDB';" | $AWK '{print $1}' | $GREP -v '^count')
...
if [ $TABLES_COUNT_AFTER_DELETE -gt 0 ]; then 
    ...
    TABLES=$($MYSQL -u $LOCAL_DB_USER $MDB -e 'show tables' | $AWK '{ print $1}' | $GREP -v '^Tables' )
