...
if [ $# -eq 0 ] || [ $# -eq 1 ]; then
    ...
    echo "$MYSQL -u cuAdmin stories < $DB_FILE"
    $MYSQL -u cuAdmin stories < $DB_FILE
    MYSQL_EXIT=$?
    if [ $MYSQL_EXIT -eq 0 ]; then
	echo "Loading triggers..."
	$MYSQL -u cuAdmin stories < $MAIN_PATH/sql/create_triggers.sql
    fi
else
    ...
    $MYSQL -u $DB_USER $PASSWORD -h $HOST $DB < $DB_FILE
    MYSQL_EXIT=$?
    if [ $MYSQL_EXIT -eq 0 ]; then
	$MYSQL -u $DB_USER $PASSWORD -h $HOST $DB < $MAIN_PATH/sql/create_triggers.sql
    fi
fi

# if we add password to DB_USER, add --password=$DB_PASS' here
