...
public class JsonEncoderDecoderInstanceLocator {
    ...
    public final HashMap<JType, String> listCollectionEncoders = new HashMap<JType, String>();
    public final HashMap<JType, String> setCollectionEncoders = new HashMap<JType, String>();
    ...
    public JsonEncoderDecoderInstanceLocator(GeneratorContext context, TreeLogger logger)
        ...
        listCollectionEncoders.put(find(Integer.class), JSON_ENCODER_DECODER_CLASS + ".LIST_INT");
        setCollectionEncoders.put(find(Integer.class), JSON_ENCODER_DECODER_CLASS + ".SET_INT");
    }
    ...
    private String getListEncoderDecoder(JType type, TreeLogger logger) throws UnableToCompleteException {
        String rc = listCollectionEncoders.get(type);
    ...
    private String getSetEncoderDecoder(JType type, TreeLogger logger) throws UnableToCompleteException {
        String rc = setCollectionEncoders.get(type);
    ...
    public String encodeExpression(JType type, String expression, Style style) throws UnableToCompleteException {
        return encodeDecodeExpression(type, expression, style, "encode", JSON_ENCODER_DECODER_CLASS + ".toJSON",
                JSON_ENCODER_DECODER_CLASS + ".toJSON", JSON_ENCODER_DECODER_CLASS
                        + ".toJSON");
    }
    ...
    public String decodeExpression(JType type, String expression, Style style) throws UnableToCompleteException {
        return encodeDecodeExpression(type, expression, style, "decode", JSON_ENCODER_DECODER_CLASS + ".toMap",
                JSON_ENCODER_DECODER_CLASS + ".toSet", JSON_ENCODER_DECODER_CLASS
                        + ".toList");
    }

    private String encodeDecodeExpression(JType type, String expression, Style style, String encoderMethod,
            String mapMethod, String setMethod, String listMethod)
            throws UnableToCompleteException {
        ...
        if (isCollectionType(clazz)) {
            ...
            if (clazz.isAssignableTo(MAP_TYPE)) {
                ...
                if (encoderDecoder != null) {
                    ...
                    if (types[1].isAssignableFrom(LIST_TYPE)) {
                        encoderDecoder = getListEncoderDecoder(types[1].isParameterized().getTypeArgs()[0], logger);
                        if (encoderDecoder != null) {
                            return mapMethod + "(" + expression + ", " + encoderDecoder + ", " +
                                    "" + "" + JSON_CLASS + ".Style." + style.name() + ")";
                        }
                    } else if (types[1].isAssignableFrom(SET_TYPE)) {
                        encoderDecoder = getSetEncoderDecoder(types[1].isParameterized().getTypeArgs()[0], logger);
                        if (encoderDecoder != null) {
                            return mapMethod + "(" + expression + ", " + encoderDecoder + ", " +
                                    "" + "" + JSON_CLASS + ".Style." + style.name() + ")";
                        }
    ...
    boolean isCollectionType(JClassType clazz) {
        return clazz != null && (clazz.isAssignableTo(SET_TYPE) || clazz.isAssignableTo(
                LIST_TYPE) || clazz.isAssignableTo(MAP_TYPE));
    }
