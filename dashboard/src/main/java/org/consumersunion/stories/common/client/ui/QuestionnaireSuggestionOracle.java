...
public class QuestionnaireSuggestionOracle extends EntitySuggestionOracle<Collection> {
    private final RpcCollectionServiceAsync collectionService;

    private Predicate<CollectionData> filter;

    @Inject
    QuestionnaireSuggestionOracle(RpcCollectionServiceAsync collectionService) {
    ...
    protected void sendRequest(final Request request, final Callback callback) {
        collectionService.getCollections(
                new RetrievePagedCollectionsParams.Builder()
                        ...
                        .withAscending(true)
                        .withQuestionnaireMask(QuestionnaireMask.QUESTIONNAIRE_MASK_QUESTIONNAIRES)
